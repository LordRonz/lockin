...
export const parseResumeTask = task({
  id: 'parse-resume',
  maxDuration: 120,
  run: async (payload: ParseResumeTaskPayload, { ctx }) => {
    logger.log('Parsing da resume!', { payload, ctx });

    const url = process.env.RESUME_PARSING_URL as string;
    const options = {
      method: 'POST',
      headers: {
        'x-rapidapi-key': process.env.RESUME_PARSING_API_KEY as string,
        'x-rapidapi-host': process.env.RESUME_PARSING_HOST as string,
